...
import { blogCategoryLabel, fetchPublicBlogPosts } from "@/lib/blog";
...
export default async function BlogIndexPage({ searchParams }: PageProps) {
  ...
  const data = await fetchPublicBlogPosts({ page, limit: 12, category: category || undefined });
  const posts = data?.posts ?? [];
  const featured =
    data?.featured && page === 1 && !category ? data.featured : null;
